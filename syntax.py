...
file = open("programs/es_primo.txt", "r")
...
for i, line in enumerate(lines, start=1):
    ...
    for word in words:
        if word in res_words:
            tkn = TokenClass(lexeme=word, tkn=res_words.get(word), pos=-1, line=i)
            tokens.append(tkn)
        elif word in arithm_operators:
            tkn = TokenClass(
                lexeme=word, tkn=arithm_operators.get(word), pos=-1, line=i
            )
            tokens.append(tkn)
        elif word in relat_operators:
            tkn = TokenClass(lexeme=word, tkn=relat_operators.get(word), pos=-1, line=i)
            tokens.append(tkn)
        elif word in log_operators:
            tkn = TokenClass(lexeme=word, tkn=log_operators.get(word), pos=-1, line=i)
            tokens.append(tkn)
        elif word in characters:
            tkn = TokenClass(lexeme=word, tkn=characters.get(word), pos=-1, line=i)
            tokens.append(tkn)
...
def generateTokensTable():
    file = open("tables/tokens_table.txt", "w")
